  ...
  return (
    <>
      <section className="relative overflow-hidden bg-[#0D1F3C] pt-32 pb-24">
        <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ background: "radial-gradient(ellipse at 60% 40%, #1B4F8A 0%, transparent 65%)" }} />
        <div className="relative z-10 mx-auto grid max-w-6xl items-end gap-12 px-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.32em] text-[#F5A623]">About Us</p>
          ...
          <div className="border-l border-white/15 pl-6">
            ...
            <Link
              to="/contact"
              className="mt-7 inline-flex bg-[#F5A623] px-7 py-3 text-xs font-semibold uppercase tracking-widest text-white transition-colors hover:bg-[#F5A623]/85"
            >
              Get a quote
            </Link>
          </div>
      ...
      <section className="bg-[#F2F0EC] py-10">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-px bg-[#1B4F8A]/10 px-6 md:grid-cols-4">
          {STATS.map((stat) => (
            <div key={stat.label} className="bg-[#F2F0EC] py-8 text-center">
              <div className="premium-font-galdgderbold text-3xl text-[#1B4F8A]">{stat.value}</div>
      ...
      <section className="bg-white py-18 sm:py-24">
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#F5A623]">How we work</p>
              <h2 className="mt-4 premium-font-galdgdersemi text-3xl leading-tight text-[#1B4F8A]">
          ...
          <div className="mt-16 border border-[#1B4F8A]/12 bg-[#1B4F8A]/5 p-6 sm:p-8">
            <h3 className="mb-6 text-sm font-bold uppercase tracking-[0.22em] text-[#1B4F8A]">Our printing process</h3>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {TIMELINE.map((step, index) => (
                <div key={step} className="border border-[#1B4F8A]/12 bg-white px-4 py-4">
